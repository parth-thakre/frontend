...
def extract_event_details(sentence, current_date=None):
    ...
    formatted_time = [convert_time_phrases(t) for t in time]
     ...
     # Clean the event descriptions to remove "am" and "pm"
    # cleaned_events = []
    # for event in events:
    #     # Remove standalone "am" or "pm" from the event description
    #     cleaned_event = re.sub(r'\bam\b|\bpm\b', '', event).strip()
    #     cleaned_events.append(cleaned_event)

    # Use the cleaned event descriptions
    # summarized_event = cleaned_events[0] if cleaned_events else convert_verbs_to_nouns(sentence)
    summarized_event = convert_verbs_to_nouns(sentence)
    # Remove 'am' and 'pm' from the summarized_event using the new function
    ...
    return result, current_date  # Return current_date along with the result


def split_sentences(sentence):
    """Split a sentence by commas, 'and', or newlines."""
