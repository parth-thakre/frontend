...
def extract_event_details(sentence: str) -> dict:
    ...
    # Handling specific timetable formats
    ...
    if time_range_match:
        ...
        return {
            "Event": event_name,
            "Date": date[0] if date else "No Date",
            "Time": time_range
        }
    
    # Return the extracted event details
    return {
        "Event": event_name,
        "Date": date[0] if date else "No Date",
        "Time": time[0] if time else "No Time"
    }
...
def events():
    ...
    try:
        event_details = process_paragraph2(text)
